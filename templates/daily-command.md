...
3. **逐項引導使用者填寫，一次一個問題**（此階段**不寫任何檔案**，僅在對話中收集回答，避免 stop hook 中途觸發導致重複提問）：
   ...
   - **腰圍 (兩個位置)**：
     - WHO midpoint（最下肋骨 + 髂嵴中點，通常肚臍上方 2-5 cm）—— 與健檢歷史可比
     - 最大腹圍（吐氣末、不收腹，軟尺上下找最大值，通常肚臍 0 至 -4 cm）—— 對內臟脂肪變化最敏感
...
9. **月份歸檔（每天執行，idempotent）**：執行 `python scripts/archive_to_monthly.py` — 將 `reviews/daily/`、`reports/daily-garmin/`、`data/garmin/` 內**非當週 ISO week** 的 daily-style 檔案移至各自的 `YYYY-MM/` 子資料夾。當週檔案保留於 root；已歸檔的檔案不再動。預設靜默通過（無需歸檔時印「無需歸檔」）。
10. **Commit & Push**：將 daily 檔案、weekly 檔案、dashboard PNG、data.js、（若週一）archive 移動結果、（若有跨週）月份歸檔移動結果一併 commit 並 push（在這一步完成後才結束對話，避免 stop hook 抱怨 untracked files）
11. **開啟 Dashboard 檢查**：執行 `open reviews/health_dashboard.png` 讓使用者確認圖表數據正確
12. 控制在 3 分鐘內完成 — 簡潔，不討論
